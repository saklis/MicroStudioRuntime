...
this.MicroVM = class MicroVM {
    constructor(meta = {}, global = {}, namespace1 = "/microstudio", preserve_ls = false) {
        ...
        if (meta.print == null) {
            meta.print = (text) => {
                ...
                return console.info('MicroVM ctor: ' + text);
            };
        }
    ...
    call(name, args = [], timeout = 3000) {
        ...
        try {
            ...
            if ((this.context.location != null) && (this.context.location.token != null)) {
                CBreak(`Error at line: ${this.context.location.token.line} column: ${this.context.location.token.column}`)
                console.info(`Error at line: ${this.context.location.token.line} column: ${this.context.location.token.column}`);
            }
            return this.storage_service.check();
        }
...
this.Runtime = class Runtime {
    ...
    start() {
        var a, i, j, k, key, l, len1, len2, len3, len4, len5, m, n, name, o, ref, ref1, ref2, ref3, ref4, ref5, s,
            value;
    ...
    startReady() {
        ...
        this.vm = new MicroVM(meta, global, namespace, false);
    ...
    drawCall() {
        ...
        try {
            ...
            this.reportWarnings();
            if (this.vm.error_info != null) {
                CBreak('drawCall error: ' + this.vm.error_info)
                err = this.vm.error_info;
                err.type = "draw";
                return this.listener.reportError(err);
            }
...
this.ProjectInterface = class ProjectInterface {
    ...
    writeMusicFile(obj, path, options, callback) {
        ...
        if (obj instanceof MicroSound) {
            loadLameJSLib(() => {
                var blob, fr, i, index, j, k, kbps, l, m, mp3Data, mp3buf, mp3encoder, ref, ref1, ref2, ref3, ref4,
                    ref5, sampleBlockSize, samples, samplesR, toindex;
                kbps = 128;
...
this.Gamepad = (function () {
    ...
    Gamepad.prototype.update = function () {
        var angle, err, i, j, k, key, l, len, len1, len2, m, n, o, pad, pad_count, pads, r, ref, ref1, ref2, ref3, ref4,
            ref5, ref6, value, x, y;
        pad_count = 0;
    ...
    AudioCore.prototype.getInterface = function () {
        ...
        return this["interface"] = {
            ...
            playSound: function (sound, volume, pitch, pan, loopit) {
                var id = getUniqueId();
                CPlaySound(sound, id, volume, pitch, pan, loopit);
                return {
                    soundName: sound,
                    uniqueId: id,
                    stop: () => {
                        CSound_Stop(this.soundName, this.uniqueId);
                        return 1;
                    },
                    setVolume: function (volume) {
                        CSound_SetVolume(this.soundName, this.uniqueId, Math.max(0, Math.min(1, volume)));
                        //return gain.gain.value = Math.max(0, Math.min(1, volume));
                    },
                    setPitch: function (pitch) {
                        CSound_SetPitch(this.soundName, this.uniqueId, Math.max(.001, Math.min(1000, pitch)));
                        //return source.playbackRate.value = Math.max(.001, Math.min(1000, pitch));
                    },
                    setPan: function (pan) {
                        CSound_SetPan(this.soundName, this.uniqueId, Math.max(-1, Math.min(1, pan)));
                    ...
                    finished: false // TODO: not implemented yet
                };
                //return audio.playSound(sound, volume, pitch, pan, loopit);
            },
            playMusic: function (music, volume, loopit) {
                CPlayMusic(music, volume, loopit);
                return {
                    musicName: music,
                    play: (function (_this) {
                        return function() {
                            CMusic_Play(this.musicName);
                            return true;
                        };
                        // return function () {
...
this.Player = class Player {
    constructor(listener) {
        ...
        //   }
        // } else {
        this.sources.main = globalThis.microScriptCode //document.getElementById("code").innerText;
        this.start();
        // }
    }
    ...
    start() {
        ...
        this.runtime = new Runtime((window.exported_project ? "" : location.origin + location.pathname), this.sources, this);
        globalThis.runtime = this.runtime; // get global ref to runtime, so I can call timer() from through C API
